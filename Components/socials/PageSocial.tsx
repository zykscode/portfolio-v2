...
const socialLinks: SocialProps = [
  ...
  config.youtube && {
    name: 'youtube',
    href: `https://www.youtube.com/zykson`,
    title: `YouTube zykson`,
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
export const PageSocial = () => {
  return (
    <div>
      {socialLinks.map((action: Socials) => (
        <Motion title={action!.title} key={action!.name} href={action!.href} name={action!.name} icon={action!.icon} />
      ))}
    </div>
  );
};
